...
Steps:
1. This script will increment the IC and BC according to args
2. Change args and run again to include other trace gases and aerosols (if necessary)
...
def xr_open_emac_dataset(fp):  # aux function, which shifts long for EMAC specific case
    print("Opening file: {}".format(fp))
    ds = xr.open_mfdataset(fp)
    ds = ds.assign_coords(lon=(((ds.lon + 180) % 360) - 180))  # EMAC lon spans [0; 360], switch to [-180; 180]
    ds = ds.sortby('lon')

    ds = ds.rename({'press':'p_rho'})
    return ds


xr_open_dataset_impl = xr_open_emac_dataset
#%% IC
if args.do_IC:
    date = dates_to_process[0]
    print("INITIAL CONDITIONS: {}".format(date))

    fp = args.emac_dir + args.emac_file_name_template.format(date_time=date.strftime('%Y%m%d_0000'), stream='WRF_bc_met')  # '%Y%m%d_%H%M'
    ...
    for mapping in mappings:
        ...
        for rule_vo in pipe_to_process:
            fp = args.emac_dir + args.emac_file_name_template.format(date_time=date.strftime('%Y%m%d_0000'), stream=mapping.output_stream)
...
if args.do_BC:
    ...
    for date in dates_to_process:
        ...
        # fp = args.emac_dir + args.emac_file_name_template.format(date_time=date.strftime('%Y%m%d_0000'), stream='ECHAM5')  # file with the pressure
        # fp = args.emac_dir + args.emac_file_name_template.format(date_time=date.strftime('%Y%m*'), stream='ECHAM5')  # MF
        # fp = args.emac_dir + args.emac_file_name_template.format(date_time=date.strftime('%Y%m%d_*'), stream='ECHAM5')  # daily MF. Due to EMAC restarts, files can split sub-daily
        # fp = args.emac_dir + args.emac_file_name_template.format(date_time=date.strftime('%Y%m%d_*'), stream='ECHAM5')  # daily MF. Due to EMAC restarts, files can split sub-daily
        fp = args.emac_dir + args.emac_file_name_template.format(date_time=date.strftime('%Y%m%d_*'), stream='WRF_bc_met')  # daily MF. Due to EMAC restarts, files can split sub-daily
        donor_ml_ds = xr_open_dataset_impl(fp).sel(time=date)
        ...
        for mapping in mappings:
            ...
            fp = args.emac_dir + args.emac_file_name_template.format(date_time=date.strftime('%Y%m%d_*'), stream=mapping.output_stream)  # daily MF
            donor_stream_ds = xr_open_dataset_impl(fp).sel(time=date)
